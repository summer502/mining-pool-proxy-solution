...
新装和重装系统都一样。装linux不需要用USB_Burning_Tool烧录。  
    ...
    **注意，以下步骤适用于Armbian 20.08及之后的版本**    
    ...
    b.<a id="install-armbian-b">在设备重新启动后</a>，显示器出现斐讯开机画面，之后会黑屏一会儿（耐心等待一下，此时正从U盘加载数据，黑屏时间与U盘读写速度有关），等待亮屏后会自动加载armbian系统  
    ...
    > 1. 如果在开机画面卡死，可以断电重启N1，拔掉键盘鼠标，只插HDMI和u盘，断电5秒后在插入电源。  
    > 2. 如果进入了android recovery界面，大概率是U盘不行，需要换个好一点的U盘试试。  
    > 3. 如果没有从U盘启动，还是启动进入了斐讯系统，则使用adb命令重启设备再一次尝试从U盘启动。  
    > 4. 如果由开机画面进入黑屏，持续长时间几分钟，可以断电重启N1，拔掉键盘鼠标，只插HDMI和u盘，断电5秒后在插入电源。  
    > 5. 如果闪屏（开机画面->黑屏->开机画面->黑屏...），需要拔掉键盘鼠标。  
    > 6. 如果出现开不了机（显示器黑屏无响应），或者在开机引导初始化加载时出现“random: crng init done”进入了`initramfs`模式，插上用U盘，断电10秒后重启，多试几次，或者重新刻录U盘再试，或者先刷低版本成功后再刷高版本，无需线刷回Android系统。  
    
    c.检查文件权限owner是否是root，如果被修改了，只能重新刻录U盘，再重新安装系统    
    ...
    **注意，只能在N1开机后插入键盘鼠标，否则可能会出现闪屏或者黑屏无法启动的情况**       
    ...
    ```
    ```
    #对反编译出的dts文件进行编辑，修改soc->bus@c1100000->interrupt-controller@9880节点的phandle属性。搜索9880，注释phandle =< 0x21 >行（也就是第262行）
    vim /root/newdtb/meson-gxl-s905d-phicomm-n1.dts
    ```
    ![image](https://user-images.githubusercontent.com/30925759/205490607-4ce27c79-c74c-4727-a02d-5ca39cbe2cdd.png)  
    ```
    #编译出新dtb文件
